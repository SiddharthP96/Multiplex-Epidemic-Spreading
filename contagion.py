...
import numpy as np
import random
#Gillespie SIR Process
def SIR_G(g1,g2,b1,b2,mu,state_vector,active_one,active_two,infected,s,r):
...
#@b1,b2 - float infection spread param values
#@ft - full time of model run
#@seed - node ID for starting seed value
#@active_one: directed graph representing activated edges in first layer
#@active_two: directed graph representing activated edges in second layer 
#@p_i: decimal value from [0-1] representing the proportion of nodes that start in the recovered state 
def gillespie_sir_sim(g1,g2,b1,b2,mu,ft,seed,p_i):
    ...
    state_vector,i_keys,r_keys,n_s= set_state(g1,g2,seed,p_i)
    ...
    n_i = len(i_keys)
    n_r = len(r_keys)
    while t < ft and n_i > 0:
        #run one step of gillespie
        n_s,n_i,n_r,dt = SIR_G(g1,g2,b1,b2,mu,state_vector,active_one,active_two,i_keys,n_s,n_r)
        #increment time
        t = t + dt
